  ...
  return (
    <ResponsiveContainer width="100%" height={180}>
      <BarChart data={data} margin={{ top: 4, right: 4, left: -18, bottom: 0 }}>
        ...
        <Tooltip
          cursor={{ fill: 'rgba(217,198,140,0.10)' }}
          contentStyle={{
            background: '#151A17',
        ...
        <Bar dataKey="value" radius={[6, 6, 0, 0]}>
          {data.map((entry, index) => (
            <Cell key={index} fill={entry.value > 0 ? '#D9C68C' : '#2A2C26'} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
    ...
    <ResponsiveContainer width="100%" height={180}>
      <LineChart data={data} margin={{ top: 8, right: 6, left: -18, bottom: 0 }}>
        ...
        <Line
          ...
          stroke="#D9C68C"
          strokeWidth={2.5}
          dot={{ r: 3, fill: '#D9C68C', strokeWidth: 0 }}
          activeDot={{ r: 5 }}
        />
      </LineChart>
    </ResponsiveContainer>
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <RadarChart data={data} outerRadius="72%">
        ...
        <Radar dataKey="value" stroke="#D9C68C" fill="#D9C68C" fillOpacity={0.25} />
      </RadarChart>
    </ResponsiveContainer>
  );
}
